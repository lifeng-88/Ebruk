...
/// API 客户端
actor VlHTTPGatewayActor {
    static let shared = VlHTTPGatewayActor()
    ...
    static func createForTesting(baseURL: String) -> VlHTTPGatewayActor {
        return VlHTTPGatewayActor(baseURL: baseURL)
    }
    ...
    private func performRequest<T: Decodable>(
        ...
        if requiresAuth {
            guard let token = accessToken, !token.isEmpty else {
                print("❌ [VlHTTPGatewayActor] requiresAuth 但未设置有效 accessToken，跳过请求: \(endpoint)")
                return .failure(.unauthorized)
            }
            ...
            do {
                ...
                if let jsonData = try? JSONSerialization.data(withJSONObject: parameters, options: .prettyPrinted),
                   let jsonString = String(data: jsonData, encoding: .utf8) {
                    print("🌐 [VlHTTPGatewayActor] Request body:")
                    print(jsonString)
                }
            } catch {
        ...
        do {
            // 调试：打印请求信息
            print("🌐 [VlHTTPGatewayActor] ========== 请求信息 ==========")
            print("🌐 [VlHTTPGatewayActor] BaseURL: \(baseURL)")
            print("🌐 [VlHTTPGatewayActor] Endpoint: \(endpoint)")
            print("🌐 [VlHTTPGatewayActor] Method: \(method.rawValue)")
            // 打印最终URL（包含查询参数）
            if let finalURL = request.url {
                print("🌐 [VlHTTPGatewayActor] Full URL: \(finalURL.absoluteString)")
            } else {
                print("🌐 [VlHTTPGatewayActor] Full URL: \(url.absoluteString)")
            }
            
            // 打印请求头
            if let headers = request.allHTTPHeaderFields, !headers.isEmpty {
                print("🌐 [VlHTTPGatewayActor] 请求头:")
                ...
                print("🌐 [VlHTTPGatewayActor] 请求头: 无")
            }
            
            // 打印请求参数
            if let parameters = parameters, !parameters.isEmpty {
                print("🌐 [VlHTTPGatewayActor] 请求参数:")
                if let jsonData = try? JSONSerialization.data(withJSONObject: parameters, options: .prettyPrinted),
                   let jsonString = String(data: jsonData, encoding: .utf8) {
                    print(jsonString)
                } else {
                ...
                print("🌐 [VlHTTPGatewayActor] 请求参数: 无")
            }
            
            // 打印请求体（如果有）
            if let body = request.httpBody {
                if let bodyString = String(data: body, encoding: .utf8) {
                    print("🌐 [VlHTTPGatewayActor] 请求体:")
                    print(bodyString)
                } else {
                    print("🌐 [VlHTTPGatewayActor] 请求体: 二进制数据 (\(body.count) bytes)")
                }
            }
            
            if let token = accessToken {
                print("🌐 [VlHTTPGatewayActor] Token: \(token.prefix(50))... (长度: \(token.count))")
            } else {
                print("🌐 [VlHTTPGatewayActor] Token: 无")
            }
            print("🌐 [VlHTTPGatewayActor] ==============================")
            
            let (data, response) = try await session.data(for: request)
            
            guard let httpResponse = response as? HTTPURLResponse else {
                print("❌ [VlHTTPGatewayActor] Invalid response type")
                return .failure(.invalidResponse)
            }
            
            // 调试：打印响应信息
            print("📡 [VlHTTPGatewayActor] ========== 响应信息 ==========")
            print("📡 [VlHTTPGatewayActor] Status Code: \(httpResponse.statusCode)")
            print("📡 [VlHTTPGatewayActor] URL: \(url.absoluteString)")
            
            // 打印响应头
            if !httpResponse.allHeaderFields.isEmpty {
                print("📡 [VlHTTPGatewayActor] 响应头:")
            ...
            if let responseString = String(data: data, encoding: .utf8) {
                print("📡 [VlHTTPGatewayActor] 响应体 (完整):")
                print(responseString)
                print("📡 [VlHTTPGatewayActor] 响应体长度: \(responseString.count) 字符, 数据大小: \(data.count) 字节")
                
                // 如果是JSON格式，尝试美化输出
                if let jsonObject = try? JSONSerialization.jsonObject(with: data, options: []),
                   let prettyData = try? JSONSerialization.data(withJSONObject: jsonObject, options: .prettyPrinted),
                   let prettyString = String(data: prettyData, encoding: .utf8) {
                    print("📡 [VlHTTPGatewayActor] 响应体 (格式化JSON):")
                    print(prettyString)
                }
            } else {
                print("⚠️ [VlHTTPGatewayActor] 响应数据不是有效的UTF-8字符串, 大小: \(data.count) 字节")
                // 尝试打印十六进制（仅前1000字节）
                let hexString = data.prefix(1000).map { String(format: "%02x", $0) }.joined(separator: " ")
                print("📡 [VlHTTPGatewayActor] 响应数据 (hex, 前1000字节): \(hexString)")
            }
            
            // 如果是错误响应，额外打印详细信息
            if httpResponse.statusCode >= 400 {
                print("❌ [VlHTTPGatewayActor] ========== 错误详情 ==========")
                print("❌ [VlHTTPGatewayActor] HTTP状态码: \(httpResponse.statusCode)")
                print("❌ [VlHTTPGatewayActor] 请求URL: \(request.url?.absoluteString ?? url.absoluteString)")
                print("❌ [VlHTTPGatewayActor] 请求方法: \(method.rawValue)")
                if let responseString = String(data: data, encoding: .utf8) {
                    print("❌ [VlHTTPGatewayActor] 错误响应体:")
                    print(responseString)
                }
                print("❌ [VlHTTPGatewayActor] ==============================")
            }
            ...
            do {
                ...
                print("✅ [VlHTTPGatewayActor] Successfully decoded response for \(url.absoluteString)")
                // 注意：解码后的数据已经在原始响应数据中打印了
                return .success(result)
            } catch {
                print("❌ [VlHTTPGatewayActor] Decoding error: \(error)")
                if let responseString = String(data: data, encoding: .utf8) {
                    print("❌ [VlHTTPGatewayActor] Failed to decode response data (完整内容):")
                    print(responseString)
                } else {
                    print("❌ [VlHTTPGatewayActor] Response data is not valid UTF-8, size: \(data.count) bytes")
                }
                // 如果解析失败，尝试解析为错误响应
                if let errorResponse = try? JSONDecoder().decode(ErrorResponse.self, from: data) {
                    return .failure(.serverError(code: httpResponse.statusCode, message: errorResponse.message))
    ...
    /// 控制台诊断：区分 DNS(-1003)、断连(-1005)、超时等，便于与系统 `nw_` 日志对照（与 `VlHTTPGatewayActor` 一致）
    private static func logURLSessionFailure(_ error: Error, label: String) {
        if let urlError = error as? URLError {
            ...
            print("❌ [VlHTTPGatewayActor] 传输失败 [\(label)] URLError rawValue=\(code) \(urlError.code) failingURL=\(failing) — \(urlError.localizedDescription)")
        } else {
            print("❌ [VlHTTPGatewayActor] 传输失败 [\(label)] \(type(of: error)): \(error.localizedDescription)")
        }
    }
    ...
    private func performFileUpload<T: Decodable>(
        ...
        do {
            print("📤 [VlHTTPGatewayActor] Uploading file: \(fileName) (\(fileData.count) bytes) to \(url.absoluteString)")
            ...
            print("📡 [VlHTTPGatewayActor] Upload response: \(httpResponse.statusCode) for \(url.absoluteString)")
            ...
            do {
                ...
                decoder.keyDecodingStrategy = .convertFromSnakeCase
                let result = try decoder.decode(T.self, from: data)
                print("✅ [VlHTTPGatewayActor] Upload successful, decoded response")
                return .success(result)
            } catch {
                print("❌ [VlHTTPGatewayActor] Upload decoding error: \(error)")
                if let responseString = String(data: data, encoding: .utf8) {
                    print("❌ [VlHTTPGatewayActor] Response data: \(responseString)")
                }
                if let errorResponse = try? JSONDecoder().decode(ErrorResponse.self, from: data) {
                    return .failure(.serverError(code: httpResponse.statusCode, message: errorResponse.message))
                }
