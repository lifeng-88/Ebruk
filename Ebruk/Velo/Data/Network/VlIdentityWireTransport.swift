...
/// 认证相关 API
struct VlIdentityWireTransport {
    static let client = VlHTTPGatewayActor.shared
    ...
    static func login(request: LoginRequest) async -> Result<LoginResponse, AppError> {
        ...
        print("🔐 [VlIdentityWireTransport] ========== 登录接口调用 ==========")
        print("🔐 [VlIdentityWireTransport] 开始调用登录接口")
        ...
        /// 公开接口：与网关约定一致 — `requiresAuth: false`（不带 Bearer）；`retryOnUnauthorized: false`（与 `/v1/refresh` 同策略，避免无意义刷新链）。
        ...
        case .success(let wrapper):
            print("✅ [VlIdentityWireTransport] 登录接口调用成功")
            print("✅ [VlIdentityWireTransport] 输出结果:")
        ...
        case .failure(let error):
            print("❌ [VlIdentityWireTransport] 登录接口调用失败")
            print("❌ [VlIdentityWireTransport] 输出结果 (错误):")
            print("   📥 错误类型: \(error)")
    ...
    static func refreshToken(refreshToken: String) async -> Result<RefreshResponseWrapper, AppError> {
        ...
        print("🔄 [VlIdentityWireTransport] 开始调用刷新Token接口")
        print("   📤 请求参数:")
        print("      - refreshToken: \(refreshToken.prefix(50))... (长度: \(refreshToken.count))")
        
        // 禁用自动重试；勿带 access Bearer（仅用 body 内 refreshToken），否则 access 为空时无法刷新且会触发 VlHTTPGatewayActor 鉴权守卫
        ...
        switch result {
        case .success(let wrapper):
            print("✅ [VlIdentityWireTransport] 刷新Token接口调用成功")
            print("   📥 服务器返回结果:")
            print("      - accessToken: \(wrapper.accessToken.prefix(50))... (长度: \(wrapper.accessToken.count))")
            print("      - refreshToken: \(wrapper.refreshToken.prefix(50))... (长度: \(wrapper.refreshToken.count))")
        case .failure(let error):
            print("❌ [VlIdentityWireTransport] 刷新Token接口调用失败")
            print("   📥 错误信息: \(error)")
            if case .serverError(let code, let message) = error {
    ...
    static func updatePushId(pushId: String) async -> Result<Bool, AppError> {
        ...
        case .success(let reply):
            print("✅ [VlIdentityWireTransport] [push_id] 上报成功 ok=\(reply.ok) \(pushId)")
            return .success(reply.ok)
        case .failure(let error):
            print("❌ [VlIdentityWireTransport] [push_id] 上报失败 \(pushId): \(error.localizedDescription)")
