//
//  VlPurchaseLedgerRepository.swift
//  Velo
//
...
/// 充值 Repository 实现
actor VlPurchaseLedgerRepository: VlPurchaseLedgerRepositoryProtocol {
    static let shared = VlPurchaseLedgerRepository()
    
    private init() {}
    
    // MARK: - VlPurchaseLedgerRepositoryProtocol
    ...
    func getPackages() async -> Result<[Package], AppError> {
        ...
        let result = await VlPurchaseWireTransport.getPackages(channelId: channelId)
    ...
    func getPayChannels() async -> Result<[PayChannel], AppError> {
        ...
        let result = await VlPurchaseWireTransport.getPayChannels(channelId: channelId)
        return result.map { $0.list }
    }
    
    func getOfficialApplePay() async -> Result<Bool, AppError> {
        let result = await VlPurchaseWireTransport.getOfficialApplePay()
        return result.map { $0.enabled }
    }
    
    func getApplePaySession(validationURL: String) async -> Result<String, AppError> {
        let result = await VlPurchaseWireTransport.getApplePaySession(validationURL: validationURL)
    ...
    func createRechargeOrder(userId: String, packageId: Int32, payChannelId: Int32, offerId: String?) async -> Result<String, AppError> {
        let result = await VlPurchaseWireTransport.createRechargeOrder(
            userId: userId,
            packageId: packageId,
    ...
    func createRedirectRechargeOrder(userId: String, packageId: Int32, payChannelId: Int32, pageUrl: String, payload: String, offerId: String?) async -> Result<CreateRechargeOrderResponse, AppError> {
        return await VlPurchaseWireTransport.createRechargeOrder(
            userId: userId,
            packageId: packageId,
            payChannelId: payChannelId,
            returnUrl: nil,
    ...
    func getOrderPaymentStatus(orderId: String) async -> Result<OrderPaymentStatusResponse, AppError> {
        return await VlPurchaseWireTransport.getOrderPaymentStatus(orderId: orderId)
    }
    ...
    func confirmRecharge(orderId: String, transactionId: String?, payChannelId: Int32, payload: String? = nil) async -> Result<ConfirmRechargeResponse, AppError> {
        return await VlPurchaseWireTransport.confirmRecharge(orderId: orderId, transactionId: transactionId, payChannelId: payChannelId, payload: payload)
    }
    ...
    func getRechargeRecords(userId: String, pageToken: String? = nil) async -> Result<(list: [RechargeRecord], nextPageToken: String?), AppError> {
        let result = await VlPurchaseWireTransport.getRechargeRecords(userId: userId, pageToken: pageToken)
    ...
    func getPaymentCards(userId: Int64) async -> Result<[PaymentCard], AppError> {
        let result = await VlPurchaseWireTransport.getPaymentCards(userId: userId)
        return result.map { $0.cards }
    }
    
    func createPaymentCard(request: CreatePaymentCardRequest) async -> Result<Int64, AppError> {
        let result = await VlPurchaseWireTransport.createPaymentCard(request: request)
        return result.map { $0.id }
    }
    
    func deletePaymentCard(id: Int64) async -> Result<Void, AppError> {
        let result = await VlPurchaseWireTransport.deletePaymentCard(id: id)
