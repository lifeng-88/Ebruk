//
//  VlIdentitySessionRepository.swift
//  Velo
//
...
/// 认证 Repository 实现
actor VlIdentitySessionRepository: VlIdentitySessionRepositoryProtocol {
    static let shared = VlIdentitySessionRepository()
    ...
    // MARK: - VlIdentitySessionRepositoryProtocol
    ...
    func login(devId: String, source: String?, channel: String?, version: String, afId: String? = nil, adId: String? = nil, afAttributionJson: String? = nil) async -> Result<AuthInfo, AppError> {
        ...
        let result = await VlIdentityWireTransport.login(request: request)
        
        switch result {
        case .success(let response):
            print("🔐 [VlIdentitySessionRepository] 登录成功，处理响应数据")
            print("   📋 响应数据:")
            ...
            do {
                ...
                await VlThirdPartyAttributionBridge.shared.markLoginCompleted()
                // 更新 VlHTTPGatewayActor 的 Token
                await VlHTTPGatewayActor.shared.setAccessToken(authInfo.accessToken)
                print("✅ [VlIdentitySessionRepository] Token 已保存到 Keychain 并更新到 VlHTTPGatewayActor")
                await MainActor.run {
                    NotificationCenter.default.post(name: .veloAuthSessionDidUpdate, object: authInfo)
                }
                if let pushId = PushManager.shared.currentPushId(), !pushId.isEmpty {
                    Task.detached(priority: .utility) {
                        _ = await VlIdentityWireTransport.updatePushId(pushId: pushId)
                    }
                }
                return .success(authInfo)
            } catch {
                print("❌ [VlIdentitySessionRepository] 保存 Token 失败: \(error)")
                return .failure(error as? AppError ?? .storageError("Failed to save auth info"))
            }
            
        case .failure(let error):
            print("❌ [VlIdentitySessionRepository] 登录失败: \(error)")
    ...
    func ensureAuthenticatedOnLaunch() async -> Result<AuthInfo, AppError> {
        if let info = await getCurrentAuthInfo() {
            print("🔐 [VlIdentitySessionRepository] ensureAuthenticatedOnLaunch: 已存在本地会话，已同步 VlHTTPGatewayActor")
            Task.detached(priority: .utility) {
        ...
        print("🔐 [VlIdentitySessionRepository] ensureAuthenticatedOnLaunch: 无本地会话，先 AF 归因再设备登录 channel=\(channel)")
    ...
    func loginWithDeviceCredentials() async -> Result<AuthInfo, AppError> {
        print("🔐 [VlIdentitySessionRepository] loginWithDeviceCredentials: refresh 失败后的设备重登（与 AuthReloginHelper 同路径）")
    ...
    func refreshToken(refreshToken: String) async -> Result<AuthInfo, AppError> {
        print("🔄 [VlIdentitySessionRepository] 开始刷新Token")
        print("   📤 输入参数:")
        print("      - refreshToken: \(refreshToken.prefix(50))... (长度: \(refreshToken.count))")
        
        let result = await VlIdentityWireTransport.refreshToken(refreshToken: refreshToken)
        
        switch result {
        case .success(let response):
            print("✅ [VlIdentitySessionRepository] 刷新Token成功，处理响应数据")
            print("   📋 服务器返回数据:")
            ...
            guard let currentAuth = currentAuth else {
                print("❌ [VlIdentitySessionRepository] 无法获取当前userid")
            ...
            do {
                try await saveAuthInfo(authInfo)
                await VlHTTPGatewayActor.shared.setAccessToken(authInfo.accessToken)
                print("✅ [VlIdentitySessionRepository] Token已保存到Keychain并更新到APIClient")
                await MainActor.run {
                    NotificationCenter.default.post(name: .veloAuthSessionDidUpdate, object: authInfo)
                }
                // 协议：token 刷新后上报 push_id（异步，与网关约定一致不阻塞）
                if let pushId = PushManager.shared.currentPushId(), !pushId.isEmpty {
                    Task.detached(priority: .utility) {
                        _ = await VlIdentityWireTransport.updatePushId(pushId: pushId)
                    }
                ...
                print("❌ [VlIdentitySessionRepository] 保存Token失败: \(error)")
                print("   📥 输出结果: 失败 - \(error)")
                return .failure(error as? AppError ?? .storageError("Failed to save auth info"))
            }
            
        case .failure(let error):
            print("❌ [VlIdentitySessionRepository] 刷新Token失败")
            print("   📥 输出结果: 失败 - \(error)")
            ...
            if Self.shouldDeviceReloginAfterRefreshFailure(error) {
                print("🔐 [VlIdentitySessionRepository] refresh 失败且可恢复（令牌失效或用户不存在等），尝试设备重登")
    ...
    func logout() async -> Result<Bool, AppError> {
        let result = await VlIdentityWireTransport.logout()
    ...
    func getCurrentAuthInfo() async -> AuthInfo? {
        // 如果内存中有，直接返回（并保证 VlHTTPGatewayActor 与内存一致，冷启动后 VlHTTPGatewayActor 可能未注入）
        ...
        // 从 Keychain 读取
        guard let accessToken = await keychain.load(key: VeloKeychainKey.accessToken),
              let refreshToken = await keychain.load(key: VeloKeychainKey.refreshToken),
              let userid = await keychain.load(key: VeloKeychainKey.userid) else {
            return nil
        }

        let authInfo = AuthInfo(userid: userid, accessToken: accessToken, refreshToken: refreshToken)
        currentAuthInfo = authInfo
        await VlHTTPGatewayActor.shared.setAccessToken(accessToken)
        return authInfo
    }
    
    func saveAuthInfo(_ authInfo: AuthInfo) async throws {
        // 保存到 Keychain
        try await keychain.save(key: VeloKeychainKey.accessToken, value: authInfo.accessToken)
        try await keychain.save(key: VeloKeychainKey.refreshToken, value: authInfo.refreshToken)
        try await keychain.save(key: VeloKeychainKey.userid, value: authInfo.userid)
    ...
    func clearAuthInfo() async {
        // 清除 Keychain
        await keychain.delete(key: VeloKeychainKey.accessToken)
        await keychain.delete(key: VeloKeychainKey.refreshToken)
        await keychain.delete(key: VeloKeychainKey.userid)
        
        // 清除内存
        currentAuthInfo = nil
        
        // 清除 VlHTTPGatewayActor 的 Token
        await VlHTTPGatewayActor.shared.setAccessToken(nil)
