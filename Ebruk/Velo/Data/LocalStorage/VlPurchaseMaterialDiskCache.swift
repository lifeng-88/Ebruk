//
//  VlPurchaseMaterialDiskCache.swift
//  Velo
//
...
/// 负责缓存充值套餐和支付渠道数据
@MainActor
class VlPurchaseMaterialDiskCache {
    static let shared = VlPurchaseMaterialDiskCache()
    ...
    private func getCachedData<T: Codable>(fileName: String, as type: T.Type) -> T? {
        ...
        do {
            ...
            print("⚠️ [VlPurchaseMaterialDiskCache] Failed to decode cached data from \(fileName): \(error)")
            return nil
        }
    }
    ...
    private func setCachedData<T: Codable>(fileName: String, data: T) {
        ...
        do {
            ...
            print("⚠️ [VlPurchaseMaterialDiskCache] Failed to cache data to \(fileName): \(error)")
        }
    }
    ...
    func clearAllCache() {
        ...
        print("🗑️ [VlPurchaseMaterialDiskCache] Cleared all cache")
    }
}
