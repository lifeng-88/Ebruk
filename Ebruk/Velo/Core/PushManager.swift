...
//  Velo
//
//  APNs device token 持久化到 `UserDefaults("velo.push_id")`（与 `PushManager` 对齐：冷启动即可 `currentPushId()`）；
//  上报 `POST /v1/push_id` 在已登录时异步执行（勿在主线程 await 网络）。失败时挂起，监听 `veloAuthSessionDidUpdate` 自动补传。
//
...
/// 上报兜底：失败标 `pendingResyncToServer`，下一次 `veloAuthSessionDidUpdate`（登录 / refresh / 启动恢复）触发后自动重发。
final class PushManager {
    static let shared = PushManager()

    /// 持久化键：与 `PushManager.pushIdKey` 等价；冷启动后立即可用，避免登录请求里 `LoginRequest.pushId == nil`。
    /// 历史模板用裸 `"push_id"`，这里用带项目前缀的 key，与 keychain / UserDefaults 现有键风格一致。
    private static let pushIdDefaultsKey = "velo.push_id"

    private let defaults = UserDefaults.standard
    private let lock = NSLock()
    /// 上一次 `POST /v1/push_id` 失败（含未登录 / 网络 / 401 链路尽头失败）：挂起；监听 `veloAuthSessionDidUpdate`
    ...
    private init() {
        ...
        NotificationCenter.default.addObserver(
            forName: .veloAuthSessionDidUpdate,
            object: nil,
            queue: nil
        ) { [weak self] _ in
    ...
    /// 关键业务成功后：未决定则弹系统权限；已允许则再 register 兜底；已拒绝则忽略（可在任务成功后按需调用）。
    ...
    func setAPNsDeviceToken(_ data: Data) {
        ...
        // UserDefaults 持久化；下次冷启动 `currentPushId()` 立即可用，
    ...
    /// 失败（未登录 / 网络异常 / `VlHTTPGatewayActor` 401 自动恢复链尽头仍 401）时记 `pendingResyncToServer`，
    /// 待 `veloAuthSessionDidUpdate` 触发后由 `resyncPushIdToServerIfPending` 自动补传一次。
    func syncPushIdToServerIfAuthenticated() async {
        ...
        guard let auth = await VlIdentitySessionRepository.shared.getCurrentAuthInfo() else {
            ...
            print("📲 [PushManager] currentPushId 已就绪但尚未登录，挂起待 veloAuthSessionDidUpdate 后补传")
            markPendingResync(true)
            return
        }
        // 与 `PushManager.reportPushIdIfAuthenticated` 对齐：每次上报前强制把 VlHTTPGatewayActor access token
        // 同步为 Keychain 里的最新值。`getCurrentAuthInfo` 命中内存时不会重设 VlHTTPGatewayActor token，若中途被
        // `clearAuthInfo` 冲为 nil（或冷启动注入时序问题）会发出空 `Bearer ` 给 OpenResty 直接 401。
        await VlHTTPGatewayActor.shared.setAccessToken(auth.accessToken)
        print("📲 [push_id] reporting to server: \(hex)")
        let result = await VlIdentityWireTransport.updatePushId(pushId: hex)
        switch result {
        case .success(let ok) where ok:
            markPendingResync(false)
        default:
            // 401 自动 refresh / 设备重登链路若全部失败，会落到这里。挂起后只需任意一次会话更新通知就会自愈。
            print("📲 [PushManager] /v1/push_id 上报未成功，挂起待下次 veloAuthSessionDidUpdate 后补传")
            markPendingResync(true)
        }
    }

    /// 与 `reportPushIdIfAuthenticated(pushId:)` 命名/语义对齐：直接传入刚拿到的 token，避免依赖 `currentPushId()`
    ...
    /// 收到 `veloAuthSessionDidUpdate`（access token 刷新 / 设备重登 / 启动恢复会话）后调用：
    ...
    private func resyncPushIdToServerIfPending() async {
        ...
        guard await VlIdentitySessionRepository.shared.getCurrentAuthInfo() != nil else { return }
        print("📲 [PushManager] 检测到 veloAuthSessionDidUpdate，补传上次失败的 push_id")
        await syncPushIdToServerIfAuthenticated()
    }
