//
//  VeloApplicationDelegate.swift
//  Velo
//
//  APNs token、`UNUserNotificationCenter` 委托、冷启动 launchOptions、DEBUG 环境变量模拟 payload。
//
...
final class VeloApplicationDelegate: NSObject, UIApplicationDelegate {
    func application(
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        UNUserNotificationCenter.current().delegate = VeloPushNotificationCenterDelegate.shared
        ...
        if let userInfo = launchOptions?[.remoteNotification] as? [AnyHashable: Any] {
            ...
            #if DEBUG
            print("📲 [VeloApplicationDelegate] didFinishLaunching：无 launchOptions.remoteNotification（正常）；推送点进应用走 UN 委托")
    ...
    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        ...
        print("📲 [VeloApplicationDelegate] didFailToRegisterForRemoteNotifications: \(error.localizedDescription) (domain=\(ns.domain) code=\(ns.code))")
        #if targetEnvironment(simulator)
        print("📲 [VeloApplicationDelegate] 模拟器：需 entitlements 含 aps-environment；Xcode 16+ 可用 .apns 文件测试推送")
        #else
    ...
    func application(
        ...
        print("📲 [VeloApplicationDelegate] didReceiveRemoteNotification \(diag) businessPayloadOK=\(ok)（系统送达回调：此处不跳转；点通知走 UN 委托）")
        // 勿在此处 `postRemotePushRouteIfParsed`：无用户点击时也会回调，会导致误跳转与重复 `push_open`。
        completionHandler(.noData)
    }

    /// 须在主线程调用：投递 `RemotePushRoute`（与 `ContentView` / `AppTabRouter` 约定一致）；与网关约定一致写入 `push_open` 埋点。
    static func postRemotePushRouteIfParsed(from userInfo: [AnyHashable: Any]) {
        ...
        Task {
            await VlClientTelemetryOutbox.shared.enqueuePushOpen(taskId: taskId, extra: extra.isEmpty ? nil : extra)
        }
        NotificationCenter.default.post(name: .veloRemotePushRoute, object: route)
    }

    #if DEBUG
    /// Scheme → Run → Arguments → Environment Variables: `VELO_DEBUG_PUSH_USERINFO` = 一行 JSON（与线上下发自定义段一致）。
    private func applyDebugPushUserInfoFromEnvironmentIfNeeded() {
        guard let raw = ProcessInfo.processInfo.environment["VELO_DEBUG_PUSH_USERINFO"]?
        ...
        guard let data = raw.data(using: .utf8),
            ...
            print("📲 [VeloApplicationDelegate] DEBUG VELO_DEBUG_PUSH_USERINFO JSON 解析失败 raw=\(String(raw.prefix(400)))")
            return
        }
        ...
        let diag = RemotePushRoute.diagnosticSummary(userInfo: userInfo)
        print("📲 [VeloApplicationDelegate] DEBUG applyPushUserInfo from VELO_DEBUG_PUSH_USERINFO \(diag)")
        DispatchQueue.main.async {
            Self.postRemotePushRouteIfParsed(from: userInfo)
        }
    }
