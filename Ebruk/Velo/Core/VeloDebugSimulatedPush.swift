...
enum VeloDebugSimulatedPush {
    static let cases: [SimPushDebugPayload] = [
        ...
        SimPushDebugPayload(
            ...
            offer_id: offer_001
            apple_product_id: com.ebruk.app.coins_20
            campaign_id: cmp_1
            ...
            route: .rechargeIncentiveNewUser(
                RechargeNewUserOfferPayload(
                    offerId: "offer_001",
                    appleProductId: "com.ebruk.app.coins_20",
                    campaignId: "cmp_1",
                    amountCentsUsd: 499,
                    baseCoins: 1000,
                    bonusCoins: 2000
    ...
    /// 与服务端 APNs `userInfo` 字段一致：调度一次本地通知，由 `VeloPushNotificationCenterDelegate.didReceive`
    ...
    private static func resolveWithRealTaskIfNeeded(_ payload: SimPushDebugPayload) async -> SimPushDebugPayload {
        ...
        let result = await VlAsyncRenderJobWireTransport.getTaskList(pageNum: Int32(1), pageSize: Int32(50))
