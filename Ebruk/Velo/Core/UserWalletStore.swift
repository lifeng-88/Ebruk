...
//  Velo
//
//  全应用共享：金币余额、充值流水、已保存支付方式（Keychain 持久化）
//
...
final class UserWalletStore: ObservableObject {
    ...
    private enum LegacyDefaultsKey {
        static let balance = "velo.wallet.coinBalance"
        static let history = "velo.wallet.transactionHistory"
        static let cards = "velo.wallet.savedCards"
    }

    private var persistenceCancellables = Set<AnyCancellable>()
    private let defaults = UserDefaults.standard
    private var isRestoringPersistence = false
    ...
    init() {
        ...
        setupPersistence()
        Task { await loadPersistedWallet() }
    }
    ...
    func syncCoinBalanceFromServer(userId: String?) async {
        ...
        let result = await VlWalletProfileWireTransport.getUserGold(userid: uid)
        await MainActor.run {
    ...
    @MainActor
    private func loadPersistedWallet() async {
        isRestoringPersistence = true

        let stored = await VeloUserKeychainStore.shared.loadWallet()
        if stored.balance != nil || stored.historyData != nil || stored.cardsData != nil {
            applyLoadedWallet(stored)
        } else {
            migrateWalletFromUserDefaultsIfNeeded()
        }
    ...
    @MainActor
    private func applyLoadedWallet(_ stored: (balance: Int?, historyData: Data?, cardsData: Data?)) {
        if let balance = stored.balance {
            coinBalance = balance
        }

        if let historyData = stored.historyData,
           let decoded = try? JSONDecoder().decode([RechargeRecordItem].self, from: historyData),
           !decoded.isEmpty {
            transactionHistory = decoded
        }

        if let cardsData = stored.cardsData,
           let decoded = try? JSONDecoder().decode([SavedPaymentCard].self, from: cardsData),
           !decoded.isEmpty {
    ...
    @MainActor
    private func migrateWalletFromUserDefaultsIfNeeded() {
        var didMigrate = false

        if let balance = defaults.object(forKey: LegacyDefaultsKey.balance) as? Int {
            coinBalance = balance
            didMigrate = true
        }

        if let data = defaults.data(forKey: LegacyDefaultsKey.history),
        ...
        if let data = defaults.data(forKey: LegacyDefaultsKey.cards),
        ...
        if didMigrate {
            defaults.removeObject(forKey: LegacyDefaultsKey.balance)
            defaults.removeObject(forKey: LegacyDefaultsKey.history)
            defaults.removeObject(forKey: LegacyDefaultsKey.cards)
            print("✅ [UserWalletStore] 已从 UserDefaults 迁移钱包数据到 Keychain")
        }
    }
    ...
    private func persist() {
        ...
        let historyData = try? JSONEncoder().encode(transactionHistory)
        let cardsData = try? JSONEncoder().encode(savedCards)

        Task {
            do {
                try await VeloUserKeychainStore.shared.saveWallet(
                    balance: balance,
                    historyData: historyData,
                    cardsData: cardsData
                )
            } catch {
                print("⚠️ [UserWalletStore] Keychain 持久化失败: \(error)")
            }
        }
    }
}
