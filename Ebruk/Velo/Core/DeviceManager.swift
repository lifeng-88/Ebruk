...
actor DeviceManager {
    ...
    #if DEBUG
    /// 联调固定 `devId`；仅当 Info.plist `isTest == 1` 时生效（见 `AppConfig.usesDebugFixedDeviceId`）。
    ...
    private static let keychainDevIdAccount = VeloKeychainKey.devId

    private init() {}

    /// 持久化 `devId`：DEBUG 且 `isTest == 1` 时用固定联调 ID；否则优先读 Keychain，首次写入优先 `identifierForVendor`，否则随机 UUID。
    func getDeviceId() async -> String {
        #if DEBUG
        if AppConfig.usesDebugFixedDeviceId {
