...
//  Velo
//
//  充值 AppsFlyer：首充 `recharge_first_success`、每次 `af_purchase`；与网关约定一致携带 `af_revenue`、`af_currency`
...
enum RechargeAFLogger {
    private static let kHasEverRechargedKeyPrefix = "velo.hasEverRechargedSuccess"
    private static let kRevenueCacheKeyPrefix = "velo.rechargeRevenueCache"
    private static let kRevenueCacheTimeKeyPrefix = "velo.rechargeRevenueCacheTime"
    ...
    /// 取出并清除缓存；过期或金额为 0 返回 nil（与 行为一致）
    ...
    private static func currentRechargeUserKey() async -> String {
        if let auth = await VlIdentitySessionRepository.shared.getCurrentAuthInfo(), !auth.userid.isEmpty {
            return auth.userid
        }
        return "anonymous"
    }
