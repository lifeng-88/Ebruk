...
//  Velo
//
//  轻量设备登录：POST /v1/login，与 Data/VlIdentityWireTransport 约定一致；基址与 `VlHTTPGatewayActor` 相同，使用 `APIBaseURL.effective`。
//
...
enum DeviceLoginService {
    ...
    /// 请求体字段与 `VlIdentityWireTransport.login` 一致；优先走 Repository，本方法仅保留作独立 URLSession 调试路径。
    static func login(
        channel: String? = "ios",
        source: String? = "app",
        afId: String? = nil,
