//
//  VlThirdPartyAttributionBridge.swift
//  Velo
//
...
actor VlThirdPartyAttributionBridge {
    static let shared = VlThirdPartyAttributionBridge()
    ...
    private let configManager = VlAttributionRemoteConfigBox.shared
    ...
    /// 标记已完成登录（由 VlIdentitySessionRepository 在登录成功时调用）
    func markLoginCompleted() {
        ...
        print("📱 [VlThirdPartyAttributionBridge] markLoginCompleted: 已标记曾完成登录")
    }
    ...
    func setAttribution(afId: String?, adId: String?, source: String?, attributionJson: String?) {
        ...
        print("📱 [VlThirdPartyAttributionBridge] setAttribution: afId=\(afId ?? "nil") source=\(source ?? "nil") jsonLen=\(attributionJson?.count ?? 0)")
        if let cont = attributionContinuation {
            attributionContinuation = nil
            cont.resume(returning: attributionResult)
            print("📱 [VlThirdPartyAttributionBridge] setAttribution: 已 resume 等待中的归因")
        }
    }
    ...
    func waitForAttributionOrTimeout() async -> AFAttributionResult? {
        if hasObtainedAttributionBefore {
            ...
            print("📱 [VlThirdPartyAttributionBridge] waitForAttributionOrTimeout: 已有归因缓存，直接返回")
            return cached
        }
        print("📱 [VlThirdPartyAttributionBridge] waitForAttributionOrTimeout: 等待归因，超时 \(Int(attributionTimeoutSeconds))s")
        ...
        let result = await withTaskCancellationHandler(operation: {
            ...
            Task { await VlThirdPartyAttributionBridge.shared.resumeAttributionWaitCancelled() }
        })
        if result != nil {
            print("📱 [VlThirdPartyAttributionBridge] waitForAttributionOrTimeout: 得到归因 afId=\(result?.afId ?? "nil") source=\(result?.source ?? "nil")")
        } else {
            print("📱 [VlThirdPartyAttributionBridge] waitForAttributionOrTimeout: 超时或无归因，返回 nil")
        }
        return result
    }
    ...
    private func resumeAttributionWaitCancelled() {
        ...
        print("📱 [VlThirdPartyAttributionBridge] resumeAttributionWaitCancelled: 已因取消 resume")
    }
    ...
    private func timeoutAttribution() async {
        if let cont = attributionContinuation {
            ...
            defaults.set(true, forKey: kAFHasObtainedAttribution)
            cont.resume(returning: getAttributionForLogin())
            print("📱 [VlThirdPartyAttributionBridge] timeoutAttribution: 超时，已 resume")
        }
    }
    ...
    func prepareForFirstLaunch(channelId: String) async -> (canLogin: Bool, attribution: AFAttributionResult?) {
        ...
        print("📱 [VlThirdPartyAttributionBridge] prepareForFirstLaunch: 开始 channel=\(effectiveChannel)")
        #if DEBUG
        if ProcessInfo.processInfo.environment["SIMULATE_AF_TIMEOUT"] == "1" {
            print("🧪 [VlThirdPartyAttributionBridge] prepareForFirstLaunch: 模拟 AF 超时 (SIMULATE_AF_TIMEOUT=1)，跳过等待，返回 nil")
            return (true, nil)
        }
        #endif
        guard let appleAppId = await configManager.getAppleAppID(channelId: effectiveChannel),
            ...
            print("⚠️ [VlThirdPartyAttributionBridge] prepareForFirstLaunch: 无 AF config，跳过 init channel=\(effectiveChannel)")
            return (true, nil)
        }
        print("📱 [VlThirdPartyAttributionBridge] prepareForFirstLaunch: 配置 AF 并 start，等待归因")
        AFSDKBridge.configure(appleAppId: appleAppId, appsFlyerDevKey: devKey)
        AFSDKBridge.start()
        let attribution = await waitForAttributionOrTimeout()
        print("📱 [VlThirdPartyAttributionBridge] prepareForFirstLaunch: 完成 attribution=\(attribution != nil ? "有" : "无")")
    ...
    func initAFAsync(channelId: String) async {
        let effectiveChannel = channelId.isEmpty ? AppConfig.buildDefaultChannelId : channelId
        print("📱 [VlThirdPartyAttributionBridge] initAFAsync: 开始 channel=\(effectiveChannel)")
        guard let appleAppId = await configManager.getAppleAppID(channelId: effectiveChannel),
              let devKey = await configManager.getAppsFlyerDevKey(channelId: effectiveChannel),
              !appleAppId.isEmpty, !devKey.isEmpty else {
            print("📱 [VlThirdPartyAttributionBridge] initAFAsync: 无 AF config，跳过 channel=\(effectiveChannel)")
            return
        }
        AFSDKBridge.configure(appleAppId: appleAppId, appsFlyerDevKey: devKey)
        AFSDKBridge.start()
        print("📱 [VlThirdPartyAttributionBridge] initAFAsync: 完成 channel=\(effectiveChannel)")
    }
}
...
private final class AFDelegateWrapper: NSObject, AppsFlyerLibDelegate {
    ...
    func onConversionDataSuccess(_ conversionInfo: [AnyHashable: Any]) {
        ...
        Task { await VlThirdPartyAttributionBridge.shared.setAttribution(afId: afId, adId: nil, source: source, attributionJson: json) }
    }
    func onConversionDataFail(_ error: Error) {
        print("📱 [AFDelegate] onConversionDataFail: \(error.localizedDescription)")
        Task { await VlThirdPartyAttributionBridge.shared.setAttribution(afId: nil, adId: nil, source: nil, attributionJson: nil) }
    }
}
#endif
