...
//  Velo
//
//  POST /v1/users/{userid}/locale：已登录时上报界面语言与时区（与网关约定一致）。
//
...
enum UserLocaleReporter {
    ...
    static func reportIfAuthenticated(reason: String = "default") async {
        ...
        let result = await VlWalletProfileWireTransport.reportUserLocale(userid: auth.userid, language: language, timeZone: timeZoneId)
        switch result {
