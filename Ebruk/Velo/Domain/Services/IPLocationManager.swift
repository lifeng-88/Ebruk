...
class IPLocationManager: ObservableObject {
    ...
    func fetchLocationInBackground() async {
        // 未登录不请求，避免 401
        let auth = await VlIdentitySessionRepository.shared.getCurrentAuthInfo()
        guard auth != nil else {
        ...
        print("🔍 [IPLocationManager] 即将请求 GET /v1/locations/ip（此后应看到 VlHTTPGatewayActor 的 🌐 请求日志）")
        let result: Result<IPLocationResponse, AppError> = await VlHTTPGatewayActor.shared.request(
            "/v1/locations/ip",
            method: .get,
            parameters: nil,
            requiresAuth: true
