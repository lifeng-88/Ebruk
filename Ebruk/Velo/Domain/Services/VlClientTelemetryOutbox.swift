//
//  VlClientTelemetryOutbox.swift
//  Velo
//
//  本地行为事件队列与批量上报策略。协议文档：协议文档/统计与埋点协议.md
//  触发条件：队列长度 ≥ N、距上次成功上报 ≥ T 秒、应用进入后台（由调用方在 scenePhase 中调用 flush）。
//  与 `VlClientTelemetryOutbox` 对齐：`template_type`、`push_open` 即时上报等。
//

import Foundation

/// 行为事件队列：内存队列 + 条数/时间/后台触发，满足条件时调用批量上报接口
actor VlClientTelemetryOutbox {
    static let shared = VlClientTelemetryOutbox()
    ...
    func enqueue(eventType: String, templateId: String, taskId: String? = nil, ts: Int64? = nil, templateType: Int? = nil, extra: [String: Any]? = nil) {
        ...
        queue.append(item)
        print("📊 [VlClientTelemetryOutbox] 事件入队: eventType=\(eventType), templateId=\(templateId), queueSize=\(queue.count), hasExtra=\(extra != nil)")
        Task { await tryFlushIfNeeded() }
    }

    /// 外部在应用进入后台时调用，立即上报当前队列
    func flush() async {
        print("📊 [VlClientTelemetryOutbox] 应用进入后台，立即上报队列（当前队列长度: \(queue.count)）")
        await doFlush()
    }
    ...
    private func tryFlushIfNeeded() async {
        ...
        if hasPaymentEvents || hasFeedbackEvents || hasPushEvents || ((shouldByCount || shouldByTime) && (!withinMinInterval || tooManyPending)) {
            if hasPaymentEvents {
                print("📊 [VlClientTelemetryOutbox] 队列含支付类事件，触发上报（queueSize=\(queue.count)）")
            } else if hasFeedbackEvents {
                print("📊 [VlClientTelemetryOutbox] 队列含反馈类事件，触发上报（queueSize=\(queue.count)）")
            } else if hasPushEvents {
                print("📊 [VlClientTelemetryOutbox] 队列含 push_open 事件，触发上报（queueSize=\(queue.count)）")
            } else {
                print("📊 [VlClientTelemetryOutbox] 满足上报条件: shouldByCount=\(shouldByCount), shouldByTime=\(shouldByTime), queueSize=\(queue.count)")
            }
            await doFlush()
        }
    }
    ...
    private func doFlush() async {
        guard !queue.isEmpty else {
            print("📊 [VlClientTelemetryOutbox] 队列为空，跳过上报")
            return
        }
        ...
        guard await VlIdentitySessionRepository.shared.getCurrentAuthInfo() != nil else {
            print("📊 [VlClientTelemetryOutbox] 无本地会话，跳过 events/batch（避免无 Authorization 被网关拒绝）；登录后下次入队或切后台会再上报")
            return
        }
        await refreshAccessTokenForStatisticsBatchIfStale()
        guard let auth = await VlIdentitySessionRepository.shared.getCurrentAuthInfo() else {
            print("📊 [VlClientTelemetryOutbox] 刷新后会话丢失，跳过 events/batch")
            return
        }
        ...
        print("📊 [VlClientTelemetryOutbox] 开始上报: batchSize=\(batch.count), channelId=\(channelId), userId=\(userId.map { "\($0)" } ?? auth.userid), deviceId=\(deviceId)")
        ...
        let result = await VlProductTelemetryAPI.reportBatch(
            channelId: channelId,
            userId: userId,
            deviceId: deviceId,
            events: batch,
        ...
        case .success(let response):
            ...
            print("✅ [VlClientTelemetryOutbox] 上报成功: accepted=\(response.accepted), rejected=\(response.rejected), 剩余队列长度=\(queue.count)")
        case .failure(let error):
            // 保留本批，下次满足条件时重试
            print("❌ [VlClientTelemetryOutbox] 上报失败: \(error.localizedDescription), 保留本批待重试（队列长度: \(queue.count)）")
        }
    }

    /// 在发 `events/batch` 前尝试刷新 access（`VlIdentitySessionRepository` 成功时会写 Keychain 并同步 `VlHTTPGatewayActor`），节流避免频繁打 `/v1/refresh`。
    private func refreshAccessTokenForStatisticsBatchIfStale() async {
        ...
        guard let info = await VlIdentitySessionRepository.shared.getCurrentAuthInfo() else { return }
        switch await VlIdentitySessionRepository.shared.refreshToken(refreshToken: info.refreshToken) {
        case .success:
            print("📊 [VlClientTelemetryOutbox] 统计上报前已刷新 access token（节流间隔 \(Int(statisticsBatchAuthRefreshMinInterval))s）")
        case .failure:
            print("📊 [VlClientTelemetryOutbox] 统计上报前 token 刷新失败，沿用当前 access token 尝试 batch")
        }
    }
}
