...
/// 支付卡管理器
@MainActor
class VlStoredCardCoordinator: ObservableObject {
    static let shared = VlStoredCardCoordinator()
    ...
    func loadPaymentCards(userId: Int64) async {
        ...
        case .success(let cards):
            ...
            print("✅ [VlStoredCardCoordinator] Loaded \(cards.count) payment cards")
        case .failure(let error):
            self.errorMessage = error.localizedDescription
            print("❌ [VlStoredCardCoordinator] Failed to load payment cards: \(error.localizedDescription)")
        }
