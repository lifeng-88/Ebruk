//
//  VlStoreKitPurchaseOrchestrator.swift
//  Velo
//
...
@MainActor
final class VlStoreKitPurchaseOrchestrator: ObservableObject {
    static let shared = VlStoreKitPurchaseOrchestrator()

    private let rechargeRepository = VlPurchaseLedgerRepository.shared
    private let authRepository = VlIdentitySessionRepository.shared
    private let mapping = VlStoreKitOrderFingerprintMap.shared
    ...
    private static func handleTransactionResult(_ result: VerificationResult<Transaction>) async {
        ...
        case .verified(let transaction):
            ...
            let orderId = VlStoreKitOrderFingerprintMap.shared.getOrderId(from: uuidString)
            if orderId.isEmpty {
                return
            }
            let payChannelId = VlStoreKitOrderFingerprintMap.shared.getPayChannelId(from: uuidString)
            ...
            case .success(let response):
                ...
                if response.success || isDuplicateSuccess {
                    ...
                    VlStoreKitOrderFingerprintMap.shared.remove(for: uuidString)
...
extension VlStoreKitPurchaseOrchestrator {
