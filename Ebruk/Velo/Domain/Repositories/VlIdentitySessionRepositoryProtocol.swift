//
//  VlIdentitySessionRepositoryProtocol.swift
//  Velo
//
...
/// 认证 Repository 协议
protocol VlIdentitySessionRepositoryProtocol {
    ...
    /// 冷启动：若 Keychain 已有会话则 `getCurrentAuthInfo` 并注入 `VlHTTPGatewayActor`；否则用设备信息调用 `login`。
    /// Token 过期后的刷新由 `VlHTTPGatewayActor` 收到 401 时经 `TokenManager` → `refreshToken` 完成；刷新失败时再走本方法同路径的设备登录。
