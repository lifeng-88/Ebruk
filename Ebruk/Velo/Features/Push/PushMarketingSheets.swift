...
//  Velo
//
//  `return_user_coins_claim` / `recharge_incentive_new_user` 全屏弹层（叠在 Tab 上，非系统 alert）。
//
...
struct PushCoinsClaimSheet: View {
    ...
    private func claimGift() {
        Task {
            ...
            let result = await VlWalletProfileWireTransport.redeemRedemptionCode(userid: uid, code: context.claimId)
            await MainActor.run {
                isClaiming = false
            }
            switch result {
...
private extension View {
    @ViewBuilder
    func veloLetterSpacing(_ points: CGFloat) -> some View {
