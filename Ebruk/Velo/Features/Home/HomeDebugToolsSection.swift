...
extension TaskListItem {
    ...
    static func veloDebugPreviewSuccess() -> TaskListItem {
        TaskListItem(
            taskId: "velo_debug_success_ui",
            taskType: 1,
            tid: "",
            totalStage: nil,
            currentStage: nil,
...
struct HomeDebugToolsSection: View {
    ...
    @State private var showDebugLogoutConfirm = false
    @State private var isDebugLoggingOut = false
    ...
    var body: some View {
                ...
                HStack {
                    ...
                    VStack(spacing: 0) {
                        ...
                        debugRow(title: "模拟推送", trailing: "›") {
                            simPushPayload = VeloDebugSimulatedPush.advanceStep(&simPushStep)
                        }
                        ...
                        debugRow(
                            title: "退出登录",
                            trailing: isDebugLoggingOut ? "…" : (auth.isAuthenticated ? "清除" : "未登录")
                        ) {
                            guard !isDebugLoggingOut else { return }
                            showDebugLogoutConfirm = true
                        }
                        .disabled(isDebugLoggingOut)
                    }
        ...
        .alert("退出登录", isPresented: $showDebugLogoutConfirm) {
            Button("取消", role: .cancel) {}
            Button("退出并清除", role: .destructive) {
                isDebugLoggingOut = true
                Task {
                    await auth.debugLogoutAndClearLocalData()
                    isDebugLoggingOut = false
                }
            }
        } message: {
            Text("将调用服务端登出，并清空 Keychain 与本地缓存（图片、视频、模板、充值数据）。不会自动重新登录。")
        }
        .alert(
            "模拟推送",
            isPresented: Binding(
        ...
        .fullScreenCover(isPresented: $showDebugGenerationSuccess) {
            NavigationView {
                GenerationSuccessView(item: .veloDebugPreviewSuccess(), onRerollSuccess: nil)
                    .environment(\.veloGenerationSuccessHostDismiss) {
        ...
        .fullScreenCover(isPresented: $showDebugGenerating) {
            ...
            .onAppear {
                VlAsyncWorkPollCoordinator.shared.debugEnterFakeGeneratingState(progress: 0.65)
            }
            .onDisappear {
                VlAsyncWorkPollCoordinator.shared.reset()
            }
