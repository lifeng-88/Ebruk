...
struct HomeTemplateDetailView: View {
    ...
    private let templateDetailCardCorner: CGFloat = 14
    ...
    private let templateDetailPickSlotCorner: CGFloat = 12
    ...
    @ViewBuilder
    private func templateDetailGenerateButtonChrome(feed: HomeFeedItem, flashActive: Bool) -> some View {
    ...
    @ViewBuilder
    private func templateDetailGenerateButton(feed: HomeFeedItem, flashActive: Bool) -> some View {
        Button(action: {
            ...
            templateDetailGenerateButtonChrome(feed: feed, flashActive: flashActive)
        }
        .buttonStyle(.plain)
    ...
    private func loadDetail() async {
        ...
        case .t1:
            let r = await VlCatalogWorkRepository.shared.getImageTemplateDetail(tid: gridItem.id)
            result = r.map { .t1($0) }
        case .t2:
            let r = await VlCatalogWorkRepository.shared.getDancingTemplateDetail(tid: gridItem.id)
            result = r.map { .t2($0) }
        case .t3:
            let r = await VlCatalogWorkRepository.shared.getVideoTemplateDetail(tid: gridItem.id)
            result = r.map { .t3($0) }
        }
        await MainActor.run {
            switch result {
...
// MARK: - SwiftUI Preview · 模板详情（离线种子）

extension HomeTemplateDetailView {
    static let previewSeedTemplateDetailId = "velo-swiftui-preview-template-detail"
