...
extension HomeFeedItem {
    /// 由 `VlCatalogWireTransport.getImageTemplates`（T1）条目构建
    ...
    /// 由 `VlCatalogWireTransport.getVideoTemplates`（T3）条目构建；`transAnimation` 非空时路径 **trans 先于** `beforePics`。
    ...
    /// 由 `VlCatalogWireTransport.getDancingTemplates`（T2）条目构建；`transAnimation` 非空时路径 **trans 先于** `beforePic`。
...
extension HomeGridCardItem {
    /// 由 `VlCatalogWireTransport.getImageTemplates`（T1）条目构建（双列网格统一比例见 `HomeGridFeedView.cellAspectRatio`）
    ...
    /// 由 `VlCatalogWireTransport.getVideoTemplates`（T3）条目构建。**首页 cell 不加载 beforePic**，仅 `transAnimation` 片段 + 成片；与 `HomeFeedItem`（含 before 时间线）不同源。
    ...
    /// 由 `VlCatalogWireTransport.getDancingTemplates`（T2）条目构建。**首页 cell 不加载 beforePic**，仅 `transAnimation` 片段 + 成片。
...
extension HomeGridCardItem {
    ...
    static let sampleGrid: [HomeGridCardItem] = [
        ...
        HomeGridCardItem(
            imageURL: URL(string: "https://lh3.googleusercontent.com/aida-public/AB6AXuC6F26Q2ytXjFDv5rEJqlADERWlbZfxKC6EOKlR3nmN4cbVc8udviSeJwy5AzI-ViOPr7e6gL5wvQE1fedlppWEAnj5RSEwiW_wGicbBK1yeR73vQIwYRh7BmdBbi4X4OVlvU4je2dnFEsyzbZ1Vro-jd9oJG9SDr5JeZD5gflg2lhxPmvDWd9gC2-TIiX3VkNglZ1xZN9w9TO5-XgIlc09E_QZlWpUmxOA-9czi2l7exxui4M6FUsMZOmYeJsrL1caniKE5QSrPM70"),
            previewVideoURL: sampleGridPreviewVideoURL,
            topTag: .hot,
            bottomLeft: .coins(80),
            aspectRatio: 3 / 5,
