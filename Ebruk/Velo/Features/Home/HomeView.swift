...
//  Image / Video / Dance：经 `VlCatalogWorkRepository` 拉取（磁盘缓存 + 后台刷新）
//
...
struct HomeView: View {
    ...
    @ObservedObject private var taskPolling = VlAsyncWorkPollCoordinator.shared

    @AppStorage("velo.home.layoutMode") private var layoutModeRaw = HomeLayoutMode.immersive.rawValue
    @AppStorage("velo.home.primaryTab") private var primaryTab = 0
    @AppStorage("velo.home.selectedTag") private var selectedTag = 0
    ...
    /// `VlCatalogWorkRepository.getCatalogs` → `/v1/catalogs`；仅 **Video** 展示二级分类条并用 `catalogId` 请求；Image / Dance 无子分类
    @State private var homeCatalogs: [Catalog] = []
    /// `VlCatalogWorkRepository.getTemplateTabs` → `/v1/template_tabs`（`homeVideoTitleId` / `homeDanceTitleId` 等仍用其 `titleId`）
    @State private var homeTemplateTabs: [TemplateTab] = []
    @AppStorage("velo.home.layoutHintSeen") private var layoutHintSeen = false
    /// 大列表滑过第 8 条后的「切小列表」引导，仅展示一次
    @AppStorage("velo.home.gridSwitchGuideSeen") private var homeGridSwitchGuideSeen = false
    ...
    private func immersiveScrollStorageKey() -> String {
        ...
        return "velo.home.scroll.immersive.\(primaryTab).\(tag)"
    }
    ...
    var body: some View {
        applyHomeViewLifecycleModifiers(to: homeViewPrimaryContent)
    }

    @ViewBuilder
    private var homeViewPrimaryContent: some View {
        NavigationView {
            ZStack {
                homeRootContent
    ...
    @ViewBuilder
    private func applyHomeViewLifecycleModifiers<Content: View>(to content: Content) -> some View {
        content
        /// 进入首页：认证与分类并行拉取（分类供子标签与 Video 请求维度）
        .task {
            await withTaskGroup(of: Void.self) { group in
                group.addTask { _ = await VlIdentitySessionRepository.shared.getCurrentAuthInfo() }
    ...
    private func loadHomeTemplateTabs() async {
        ...
        let result = await VlCatalogWorkRepository.shared.getTemplateTabs(locale: locale)
        await MainActor.run {
            switch result {
    ...
    private func loadHomeCatalogs(forceRefresh: Bool) async {
        let locale = appLanguage.templateAPICatalogLocaleIdentifier
        let result = await VlCatalogWorkRepository.shared.getCatalogs(locale: locale, forceRefresh: forceRefresh)
        await MainActor.run {
            switch result {
            case .success(let list):
                homeCatalogs = list
    ...
    private func fetchImageTemplates(reset: Bool) async {
        ...
        let result = await VlCatalogWorkRepository.shared.getImageTemplates(pageNum: page, pageSize: homeListPageSize)
    ...
    private func fetchVideoTemplates(reset: Bool) async {
        if reset {
            ...
            let result = await VlCatalogWorkRepository.shared.getVideoTemplates(
                pageNum: page,
    ...
    private func fetchDanceTemplates(reset: Bool) async {
        ...
        let result = await VlCatalogWorkRepository.shared.getDancingTemplates(
            pageNum: page,
            pageSize: homeListPageSize,
...
extension Notification.Name {
    ...
    static let homeRequestPrimaryGenerate = Notification.Name("velo.home.requestPrimaryGenerate")
}
