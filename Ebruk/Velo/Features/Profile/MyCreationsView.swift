...
struct MyCreationsView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                VeloTrackedText.text(AppLanguageStore.localized("my.creations.title"), size: 15, weight: .heavy, tracking: 1.2, color: AppTheme.primary)
            }
        }
        ...
        HStack(spacing: 8) {
            ForEach(CreationsFilterTab.allCases, id: \.rawValue) { tab in
                ...
                Button {
                    ...
                    VeloTrackedText.text(tab.title, size: 10, weight: .heavy, tracking: 1)
    ...
    private var noticeBanner: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            VeloTrackedText.text(AppLanguageStore.localized("my.creations.banner.expire"), size: 9, weight: .semibold, tracking: 0.6, color: AppTheme.outlineVariant)
    ...
    private func load(reset: Bool) async {
        ...
        let result = await VlAsyncRenderJobWireTransport.getTaskList(pageNum: 1, pageSize: pageSize, status: status)
        switch result {
        ...
        case .success(let firstResp):
            ...
            while collected.isEmpty,
                ...
                page += 1
                let next = await VlAsyncRenderJobWireTransport.getTaskList(pageNum: page, pageSize: pageSize, status: status)
    ...
    private func loadMoreIfNeeded() async {
        ...
        let next = pageNum + 1
        let result = await VlAsyncRenderJobWireTransport.getTaskList(
            pageNum: next,
            pageSize: pageSize,
    ...
    private func retryTask(_ item: TaskListItem) async {
        ...
        let result = await VlAsyncRenderJobWireTransport.createTask(req)
        await MainActor.run {
            retryingTaskId = nil
            if case .success(let resp) = result {
                Task {
...
private struct CreationGridCard: View {
    ...
    private func successChrome(showSuccessPill: Bool, showExpiredOverlay: Bool) -> some View {
        ZStack {
            ...
            VStack {
                ...
                Spacer()
                if showExpiredOverlay {
                    VeloTrackedText.text(AppLanguageStore.localized("my.creations.expired"), size: 12, weight: .heavy, tracking: 1.2, color: .white)
                        .padding(.horizontal, 16)
                        .padding(.vertical, 8)
    ...
    private var successPill: some View {
        HStack(spacing: 6) {
            ...
            VeloTrackedText.text(AppLanguageStore.localized("my.creations.success_pill"), size: 9, weight: .heavy, tracking: 0.8, color: .white)
        }
        .padding(.horizontal, 10)
        .padding(.vertical, 5)
    ...
    private var failedLayer: some View {
        VStack(spacing: 10) {
            ...
            Text(AppLanguageStore.localized("my.creations.failure"))
                ...
                .foregroundStyle(.white)
            VeloTrackedText.text(AppLanguageStore.localized("my.creations.refunded"), size: 9, weight: .heavy, tracking: 1, color: AppTheme.outlineVariant)
            Spacer(minLength: 0)
            Button(action: {
                ...
                Group {
                    if isRetrying {
                        ...
                        VeloTrackedText.text(AppLanguageStore.localized("my.creations.tap_retry"), size: 10, weight: .heavy, tracking: 0.8, color: AppTheme.primary)
                    }
                }
                .padding(.vertical, 8)
                .frame(maxWidth: .infinity)
...
struct CreationDetailView: View {
    ...
    private func loadItemFromServer() async {
        ...
        let result = await VlAsyncRenderJobWireTransport.getTask(taskId: initialTaskId)
        await MainActor.run {
            loadingItem = false
            switch result {
