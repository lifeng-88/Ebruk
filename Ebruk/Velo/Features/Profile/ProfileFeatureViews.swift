...
struct MyLikesView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .principal) {
                VeloTrackedText.text(AppLanguageStore.localized("my.likes.title"), size: 16, weight: .heavy, tracking: 1.5, color: AppTheme.primary)
            }
        }
    ...
    private func gridCard(for entry: LocalFavoriteEntry) async -> HomeGridCardItem? {
        switch entry.kind {
        case .t1:
            let r = await VlCatalogWorkRepository.shared.getImageTemplateDetail(tid: entry.templateId)
            if case .success(let t) = r { return HomeGridCardItem(imageTemplate: t) }
        case .t2:
            let r = await VlCatalogWorkRepository.shared.getDancingTemplateDetail(tid: entry.templateId)
            if case .success(let t) = r { return HomeGridCardItem(dancingTemplate: t) }
        case .t3:
            let r = await VlCatalogWorkRepository.shared.getVideoTemplateDetail(tid: entry.templateId)
        ...
        HStack(spacing: 10) {
            ForEach(options, id: \.0) { tab, titleKey in
                ...
                Button {
                    ...
                    VeloTrackedText.text(AppLanguageStore.localized(titleKey), size: 10, weight: .heavy, tracking: 1.2)
...
struct ProfileSettingsView: View {
    ...
    @AppStorage("velo.profile.productUpdates") private var productUpdatesOn = false
    ...
    private func settingsSection(title: String, @ViewBuilder content: () -> some View) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            VeloTrackedText.text(title.uppercased(), size: 10, weight: .semibold, tracking: 3, color: AppTheme.outlineVariant)
...
struct LanguageSettingsView: View {
    ...
    private func title(for option: AppLanguagePreference) -> String {
        ...
        case .simplifiedChinese:
            return AppLanguageStore.localized("language.option.simplified_chinese")
    ...
    private func subtitle(for option: AppLanguagePreference) -> String {
        ...
        case .simplifiedChinese:
            return AppLanguageStore.localized("language.option.simplified_chinese.subtitle")
        case .traditionalChinese:
