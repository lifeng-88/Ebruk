...
struct UserAgreementView: View {
    ...
    private var versionPill: some View {
        VeloTrackedText.text(versionBadgeText, size: 10, weight: .heavy, tracking: 1.0, color: AppTheme.onSurfaceVariant)
    ...
    private func legalSection(number: String, title: String, body: String) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .center, spacing: 10) {
                ...
                VeloTrackedText.text("\(number). \(title)", size: 12, weight: .heavy, tracking: 0.6, color: AppTheme.primary)
            }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .center, spacing: 10) {
                RoundedRectangle(cornerRadius: 2, style: .continuous)
                    .fill(AppTheme.primary)
                    .frame(width: 32, height: 3)
                VeloTrackedText.text("3. USER CONDUCT", size: 12, weight: .heavy, tracking: 0.6, color: AppTheme.primary)
            }
...
struct PrivacyPolicyView: View {
    ...
    private var lastUpdatedPill: some View {
        VeloTrackedText.text(lastUpdatedBadge, size: 9, weight: .heavy, tracking: 0.8, color: AppTheme.onSurface.opacity(0.88))
            .padding(.horizontal, 14)
            .padding(.vertical, 7)
            .background(AppTheme.surfaceContainerHigh.opacity(0.95))
            .clipShape(Capsule())
    ...
    private func privacySection(section: String, heading: String, content: PrivacySectionContent) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            VeloTrackedText.text(String(format: AppLanguageStore.localized("legal.section_prefix"), section), size: 10, weight: .heavy, tracking: 1.0, color: AppTheme.primary)
    ...
    private var inquiriesFooter: some View {
        VStack(alignment: .leading, spacing: 10) {
            VeloTrackedText.text(AppLanguageStore.localized("legal.inquiries"), size: 10, weight: .heavy, tracking: 1.2, color: AppTheme.onSurfaceVariant.opacity(0.85))
...
struct FeedbackCenterView: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    NavigationLink {
                        ...
                        VeloTrackedText.text(AppLanguageStore.localized("feedback.view_history"), size: 11, weight: .semibold, tracking: 1.2, color: Color.white.opacity(0.92), serif: true)
    ...
    private func goldSectionTitle(_ text: String) -> some View {
        VeloTrackedText.text(text.uppercased(), size: 11, weight: .semibold, tracking: 1.6, color: AppTheme.secondary, serif: true)
    }
    ...
    private func submitFeedback() {
        ...
        Task {
            ...
            let result = await VlSupportTicketWireTransport.submitFeedback(request: request)
            await MainActor.run {
                isSubmitting = false
                switch result {
                case .success:
...
struct FeedbackHistoryView: View {
    ...
    private var summaryRow: some View {
        HStack {
            VeloTrackedText.text(AppLanguageStore.localized("feedback.active_submissions"), size: 10, weight: .semibold, tracking: 1.2, color: AppTheme.outlineVariant)
            Spacer()
    ...
    private func feedbackCard(_ item: FeedbackItem) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top) {
                VeloTrackedText.text(Self.categoryBannerLabel(item.category), size: 10, weight: .heavy, tracking: 0.8, color: Self.categoryAccentColor(item.category))
                    .lineLimit(2)
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack(spacing: 6) {
                    Image(systemName: "checkmark.seal.fill")
                        .font(.system(size: 12))
                        .foregroundStyle(AppTheme.primary)
                    VeloTrackedText.text(AppLanguageStore.localized("feedback.official_reply"), size: 10, weight: .heavy, tracking: 1, color: AppTheme.primary.opacity(0.95))
                }
                Text(text)
    ...
    private func statusBadge(status: String) -> some View {
        ...
        return VeloTrackedText.text(s.title, size: 9, weight: .heavy, tracking: 0.5, color: s.foreground)
    ...
    private var endOfHistoryFooter: some View {
        VStack(spacing: 10) {
            ...
            VeloTrackedText.text(AppLanguageStore.localized("feedback.end_history"), size: 10, weight: .bold, tracking: 2, color: AppTheme.outlineVariant.opacity(0.65))
        }
        .frame(maxWidth: .infinity)
        .padding(.top, 28)
        .padding(.bottom, 8)
    ...
    private func load(isRefresh: Bool) async {
        ...
        let result = await VlSupportTicketWireTransport.listFeedbacks(pageSize: 40)
        await MainActor.run {
            if !isRefresh {
                loading = false
            }
