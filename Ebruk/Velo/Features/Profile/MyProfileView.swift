...
struct MyProfileView: View {
    ...
    /// 目标视图是否已自带 `veloLocalizedNavigationBackButton`。新增此类目标时同步更新。
    ...
    private func profileSection<Content: View>(title: String, @ViewBuilder content: () -> Content) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            VeloTrackedText.text(title, size: 10, weight: .semibold, tracking: 2.2, color: AppTheme.outlineVariant)
    ...
    /// 第 1 次点击复制 ID；连续 10 次（2s 内无中断则重新计数）触发 `VlIdentityWireTransport` RefreshToken
    ...
    private func performDebugRefreshTokenFromSecretTaps() async {
        ...
        guard let info = await VlIdentitySessionRepository.shared.getCurrentAuthInfo() else {
            await MainActor.run {
        ...
        let result = await VlIdentitySessionRepository.shared.refreshToken(refreshToken: info.refreshToken)
        await MainActor.run {
            switch result {
            case .success:
