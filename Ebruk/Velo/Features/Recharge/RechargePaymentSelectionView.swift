...
//  Velo
//
//  Select Payment：TOTAL AMOUNT、币拆分；支付方式来自 `/v3/pay_channels`（VlCheckoutChannelRegistry）
//
...
struct RechargePaymentSelectionView: View {
    ...
    @ObservedObject private var payChannelManager = VlCheckoutChannelRegistry.shared
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 22) {
                        ...
                        if let pc = selectedPayChannel, pc.isApplePay, !appStorePaymentsAllowed {
                            HStack(alignment: .top, spacing: 10) {
                                Image(systemName: "exclamationmark.triangle.fill")
                                    .font(.system(size: 16))
                                Text(VlStoreKitPurchaseOrchestrator.appStorePaymentsDisabledMessage)
                        ...
                        VeloTrackedText.text(AppLanguageStore.localized("recharge.payment.method_section"), size: 10, weight: .bold, tracking: 2, color: AppTheme.outlineVariant)
                        ...
                        Button {
                            ...
                            HStack(spacing: 10) {
                                ...
                                VeloTrackedText.text(isPurchasing ? AppLanguageStore.localized("recharge.processing") : AppLanguageStore.localized("recharge.payment.btn_now"), size: 13, weight: .heavy, tracking: 1.8)
                            }
        ...
        .onAppear {
            appStorePaymentsAllowed = VlStoreKitPurchaseOrchestrator.deviceAllowsInAppPurchases()
        }
    ...
    private var totalAmountCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            VeloTrackedText.text(AppLanguageStore.localized("recharge.payment.total_amount"), size: 10, weight: .bold, tracking: 2, color: AppTheme.outlineVariant)
                ...
                HStack(alignment: .center, spacing: 8) {
                    ...
                    if channel.isCreditCard {
                        Button {
                            ...
                            VeloTrackedText.text(AppLanguageStore.localized("recharge.payment.add_card_short"), size: 11, weight: .heavy, tracking: 0.5, color: AppTheme.primary)
                                .padding(.horizontal, 8)
                                .padding(.vertical, 4)
                ...
                if channel.isApplePay {
                    Text(
                        appStorePaymentsAllowed
                            ...
                            : VlStoreKitPurchaseOrchestrator.appStorePaymentsDisabledMessage
                    )
                    .font(.system(size: 10, weight: .medium))
