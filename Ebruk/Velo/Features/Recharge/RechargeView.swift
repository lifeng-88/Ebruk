...
struct RechargeView: View {
    ...
    var body: some View {
        ...
        NavigationView {
            ZStack(alignment: .top) {
                ...
                .veloScrollIndicatorsHidden()
                .veloScrollBounceBasedOnSize()
                .veloListScrollContentHidden()
                .veloScrollClipDisabledIfAvailable()
                .refreshable {
                    await loadPackages()
                }
            ...
            .veloNavigationBarBackground(RechargeDesign.background)
            .toolbar {
                ToolbarItem(placement: .principal) {
                    Text(AppLanguageStore.localized("recharge.list_title"))
    ...
    private func queryBrowserRedirectPaymentIfNeeded(isUserInitiated: Bool) async {
        ...
        let result = await VlPurchaseLedgerRepository.shared.getOrderPaymentStatus(orderId: orderId)
        switch result {
    ...
    private var rechargeListBody: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(spacing: 8) {
                ...
                VeloTrackedText.text(AppLanguageStore.localized("recharge.secure_footer"), size: 9, weight: .bold, tracking: 2, color: Color.white.opacity(0.72))
            }
            .frame(maxWidth: .infinity)
            .padding(.top, 16)
            .padding(.bottom, 4)
        ...
        if isLoadingPackages && packages.isEmpty {
            ...
            VStack(spacing: 14) {
                ...
                Button {
                    ...
                    VeloTrackedText.text(AppLanguageStore.localized("common.retry"), size: 12, weight: .heavy, tracking: 1.2)
            ...
            LazyVStack(spacing: RechargeLayout.listRowSpacing) {
    ...
    private func loadPackages() async {
        ...
        let result = await VlPurchaseLedgerRepository.shared.getPackages()
        await MainActor.run {
    ...
    private func packageListCell(_ pkg: RechargePackageModel, index: Int) -> some View {
        ...
        let listCorner = RechargeLayout.listRowCorner
        let cardHeight = RechargeLayout.packageCardFixedHeight
        let hPad = RechargeLayout.packageRowHorizontalPadding
        let vPad = RechargeLayout.packageCardInnerVerticalPadding
        return Button {
            selectedId = pkg.id
            ...
            HStack(alignment: .center, spacing: 12) {
                rechargePackageLeadingCoinOrb(featured: showFeaturedChrome)
                ...
                VStack(alignment: .trailing, spacing: 2) {
                    ...
                    if let original = pkg.originalPrice {
                        Text(original)
                            .font(.system(size: 12, weight: .medium))
                            .foregroundColor(AppTheme.outlineVariant.opacity(0.78))
                            .strikethrough(true)
                            .lineLimit(1)
                            .minimumScaleFactor(0.85)
                    }
            ...
            .background(rechargePackageRowBackground(corner: listCorner, featured: showFeaturedChrome))
            .overlay(rechargePackageRowStrokeOverlay(corner: listCorner, featured: showFeaturedChrome))
            .modifier(RechargePackageRowShadowModifier(featured: showFeaturedChrome))
                    ...
                    Text(String(format: AppLanguageStore.localized("recharge.badge.free_percent"), Int64(pct)))
                        ...
                        .background(
                            Capsule(style: .continuous)
                                .fill(
                                    LinearGradient(
                                        colors: [RechargeDesign.pink, RechargeDesign.pink.opacity(0.88)],
                                        startPoint: .leading,
                                        endPoint: .trailing
                                    )
                                )
                        ...
                        .shadow(
                            color: RechargeDesign.pink.opacity(0.45),
                            radius: 8,
                            y: 2
                        )
                        .padding(.trailing, 8)
    ...
    @ViewBuilder
    private func rechargePackageLeadingCoinOrb(featured: Bool) -> some View {
    ...
    @ViewBuilder
    private func rechargePackageRowBackground(corner: CGFloat, featured: Bool) -> some View {
        let shape = RoundedRectangle(cornerRadius: corner, style: .continuous)
        shape.fill(featured ? RechargeDesign.cardFillElevated : RechargeDesign.cardFill)
    }

    @ViewBuilder
    private func rechargePackageRowStrokeOverlay(corner: CGFloat, featured: Bool) -> some View {
        let shape = RoundedRectangle(cornerRadius: corner, style: .continuous)
        shape.stroke(
            LinearGradient(
                colors: featured
    ...
    /// `payChannel.isApplePay` → `VlStoreKitPurchaseOrchestrator`；否则信用卡确认或重定向支付链接（与接口返回的 `pay_channel_id` 一致）
    private func completePurchase(package: RechargePackageModel, payChannel: PayChannel) async {
        ...
        guard VlStoreKitPurchaseOrchestrator.deviceAllowsInAppPurchases() else {
            await MainActor.run {
                paymentOutcome = .failed(message: VlStoreKitPurchaseOrchestrator.appStorePaymentsDisabledMessage)
            }
            return
        }
        await MainActor.run { isPurchasing = true }
        let iapChannelId = domain.iapPayChannelId ?? payChannel.id
        let result = await VlStoreKitPurchaseOrchestrator.shared.runIAPPurchaseFlow(package: domain, payChannelId: iapChannelId, offerId: offerId)
        await MainActor.run {
            isPurchasing = false
            checkoutPackage = nil
        }
        if result.success, result.gold > 0 {
            /// `VlStoreKitPurchaseOrchestrator` 已在 confirm 后用 `balance` 或 `refreshBalance` 更新钱包，省略二次拉取
    ...
    private func completeNonIAPPurchase(
        ...
        if useCreditCardFlow {
            let orderResult = await VlPurchaseWireTransport.createRechargeOrder(
                userId: uid,
                packageId: domain.id,
                payChannelId: payChannelId,
                returnUrl: nil,
            ...
            case .success(let resp):
                ...
                await VlStoredCardCoordinator.shared.loadPaymentCards(userId: uid64)
                guard let card = VlStoredCardCoordinator.shared.defaultCard else {
                    await MainActor.run {
                        isPurchasing = false
                ...
                let confirmResult = await VlPurchaseLedgerRepository.shared.confirmRecharge(
                    orderId: orderId,
                    transactionId: "",
                    payChannelId: payChannelId,
                    payload: payload
        ...
        let redirectResult = await VlPurchaseWireTransport.createRechargeOrder(
            userId: uid,
            packageId: domain.id,
            payChannelId: payChannelId,
            returnUrl: nil,
