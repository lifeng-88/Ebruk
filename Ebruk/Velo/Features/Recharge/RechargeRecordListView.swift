...
struct RechargeRecordListView: View {
    ...
    private func loadFirstPage() async {
        ...
        let result = await VlPurchaseLedgerRepository.shared.getRechargeRecords(userId: uid, pageToken: nil)
        await MainActor.run {
            isLoading = false
            switch result {
    ...
    private func loadMore() async {
        ...
        let result = await VlPurchaseLedgerRepository.shared.getRechargeRecords(userId: uid, pageToken: token)
        await MainActor.run {
            isLoadingMore = false
            switch result {
            case .success(let tuple):
    ...
    private func statusBadge(_ status: RechargeRecordStatus) -> some View {
        VeloTrackedText.text(status.rawValue.capitalized, size: 10, weight: .bold, tracking: 0.8, color: foreground(for: status))
