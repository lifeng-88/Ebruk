    ...
    private func successCard(totalCoins: Int, bonusCoins: Int, newBalanceFormatted: String?) -> some View {
        VStack(spacing: 0) {
            HStack(alignment: .top, spacing: 6) {
                ...
                VeloTrackedText.text(AppLanguageStore.localized("recharge.result.title"), size: 13, weight: .heavy, tracking: 1.4, color: headerLavender)
            ...
            Button(action: {
                ...
                Group {
                    if primaryShowsProgress {
                        ...
                        VeloTrackedText.text(AppLanguageStore.localized("common.ok"), size: 14, weight: .heavy, tracking: 1.2)
                            .foregroundStyle(.white)
                    }
                }
                .frame(maxWidth: .infinity)
