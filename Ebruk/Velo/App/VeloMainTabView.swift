...
struct VeloMainTabView: View {
    ...
    /// 与网关约定一致：推送营销弹窗叠在当前 Tab 上（非系统 `alert`），底层界面仍可见。
    @ViewBuilder
        ...
        return Button {
            ...
            VStack(spacing: 4) {
                Image(systemName: isSelected ? tab.icon : tab.iconInactive)
                    .font(.system(size: 22))
                VeloTrackedText.text(tab.title.uppercased(), size: 9, weight: .heavy, tracking: 0.8)
            }
    ...
    /// iOS 16+ 用 `continuous` 与全局圆角一致；iOS 15 用 `VeloTopRoundedRectangle`
    private func topTabBarRoundedShape() -> AnyShape {
        ...
        return AnyShape(VeloTopRoundedRectangle(cornerRadius: MainTabBarChrome.topCornerRadius))
    }
}
