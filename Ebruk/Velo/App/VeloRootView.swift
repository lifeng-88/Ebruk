...
struct VeloRootView: View {
    ...
    @AppStorage("velo.welcomeBonusShown") private var welcomeBonusShown = false
    ...
    var body: some View {
        VeloMainTabView()
            ...
            .onReceive(NotificationCenter.default.publisher(for: .veloAuthSessionDidUpdate)) { note in
            ...
            .onReceive(NotificationCenter.default.publisher(for: .veloRemotePushRoute)) { note in
                ...
                Task { @MainActor in
                    if auth.isAuthenticated {
                        ...
                        print("📲 [VeloRootView] 收到推送路由但未登录，暂存等登录后再分发: \(route)")
            ...
            .onChange(of: auth.isAuthenticated) { isAuthed in
                if isAuthed {
                    ...
                    if let pending = pendingRemotePushRoute {
                        ...
                        print("📲 [VeloRootView] 登录成功，补放暂存的推送路由: \(pending)")
            ...
            .task {
                ...
                let hasStoredSession = await VlIdentitySessionRepository.shared.getCurrentAuthInfo() != nil
                if hasStoredSession {
                ...
                VlStoreKitPurchaseOrchestrator.shared.startListening()
                evaluateWelcomeBonus()
            }
            ...
            .onChange(of: scenePhase) { phase in
                if phase == .active {
                    ...
                    Task { await VlClientTelemetryOutbox.shared.flush() }
                }
            }
    }
