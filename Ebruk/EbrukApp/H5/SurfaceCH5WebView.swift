...
@MainActor
final class SurfaceCH5WebViewModel: ObservableObject {
    @Published var isReady = false
    @Published var errorMessage: String?

    let pageURL: URL
    private(set) lazy var bridge = SurfaceCH5Bridge(viewModel: self)
    ...
    init(pageURL: URL) {
        ...
        SurfaceCH5Config.configure(pageURL: pageURL)
    }
    ...
    func loadIfNeeded() {
        ...
        didLoad = true
        SurfaceCH5PaymentManager.shared.startListening()
        Task {
            await SurfaceCH5AFManager.shared.initAFAsync(channelId: SurfaceCH5Config.channel)
    ...
    private func loadResolvedURL() async {
        ...
        if SurfaceCH5Config.debugLogging {
            print("🌐 [SurfaceCH5] load \(url.absoluteString)")
        }
        #endif
        load(url: url)
    }
    ...
    private func makeWebView() -> WKWebView {
        ...
        contentController.add(bridge, name: SurfaceCH5Bridge.messageName)

        let configuration = WKWebViewConfiguration()
        configuration.userContentController = contentController
        configuration.defaultWebpagePreferences.allowsContentJavaScript = true
        configuration.allowsInlineMediaPlayback = true
        configuration.setURLSchemeHandler(SurfaceCH5MediaCacheSchemeHandler(), forURLScheme: SurfaceCH5MediaCacheSchemeHandler.scheme)

        let webView = WKWebView(frame: .zero, configuration: configuration)
        SurfaceCH5Config.configureWebViewInspectability(webView)
...
struct SurfaceCH5WebView: UIViewRepresentable {
    @ObservedObject var viewModel: SurfaceCH5WebViewModel
