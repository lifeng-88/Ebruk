...
extension Notification.Name {
    static let surfaceCH5PushPayloadReceived = Notification.Name("surfaceCH5PushPayloadReceived")
    static let surfaceCH5PushTokenUpdated = Notification.Name("surfaceCH5PushTokenUpdated")
}

final class SurfaceCH5PushManager {
    static let shared = SurfaceCH5PushManager()
...
""")
    ...
    func updateDeviceToken(_ deviceToken: Data) {
        ...
        DispatchQueue.main.async {
            ...
            NotificationCenter.default.post(
                name: .surfaceCH5PushTokenUpdated,
                object: nil,
    ...
    private func registerForRemoteNotifications(completion: @escaping ([String: Any]) -> Void) {
        DispatchQueue.main.async {
            if let veloToken = PushManager.shared.currentPushId(), !veloToken.isEmpty {
                self.defaults.set(veloToken, forKey: self.tokenKey)
                Self.logDeviceToken(veloToken, source: "push.register (velo cache)")
